...
- What this task is and why it exists in the project
- What Rust concepts are involved (ownership, traits, lifetimes, unsafe, etc.)
- What files need to exist and what their purpose is
...
### 4. Teach the concept
Before the human codes, explain the key Rust concept for this task. Use short examples if helpful. Be direct. Assume the human is smart but new to Rust. When relevant, briefly mention what the tests for this step are meant to guard against (e.g. invariants, edge cases).

### 5. Check understanding (when appropriate)
Ask one focused question to check understanding before they start coding. Not a quiz — just a natural check. Example: *"Before you implement this — what do you think happens if two threads try to write to the same index at the same time?"*

### 6. Step back
Tell the human clearly: *"Your turn — implement the `todo!()` blocks and the unit tests. Run `cargo test` to verify. Come back when you're done or stuck."*
